...
function setupEventListeners() {
  // User type radio buttons (NOW INCLUDES TEACHER)
...
function toggleGuideInfo(userType) {
  ...
  if (userType === "guide") {
    ...
    guideFields.forEach((field) => {
      if (field.name !== "description") field.required = true;
    });

  } else {
    guideInfo.classList.add("hidden");
    const guideFields = guideInfo.querySelectorAll("input, select, textarea");
    guideFields.forEach((field) => (field.required = false));
  }
}
...
function updateRegisterSubtitle(userType) {
  ...
  // ADDED TEACHER SUBTITLE
  const text =
    userType === "guide"
      ? t("professionalGuide")
      : userType === "teacher"
      ? t("professionalTeacher") // <- YOU SHOULD ADD THIS TO YOUR TRANSLATION FILE
      : t("regularUser");
...
function handleRegistration() {
  ...
  // ADD TEACHER DATA (if needed)
  if (userType === "teacher") {
    registrationData.teacherInfo = {
